...
const Up = Vector3.UP
var gravity: = 10.0
const SNAPDIR = Vector3.DOWN
var SNAPLEN = 0.6
const MAXFALLSPEED = 10 / 2
const MAXSPEED := 1.8
var SUPERSPEED := 2
var WAVESPEED = 200 / 2
const ATKMSPEED = 10 / 2
const ZOOMSPEED = 300
var MSPEED
const floormax = deg2rad(48)
const JUMPf = 4
var yspeed = 0
...
var slash = preload("res://Objects/Air_Slash3D.tscn")
var sgr = preload("res://Objects/Shotgunrotary3D.tscn")
var dust = preload("res://Subrooms/dust3D.tscn")
var atkfx = preload("res://Stages/Attackfx3d.tscn")
...
func _ready():
	...
	Globals.checkpX = translation.x
	Globals.checkpY = translation.y
...
func _physics_process(delta)->void :
	...
	Globals.playerpos = global_translation
	
	Globals.playerspriteflip = $Rspr.flip_h
	
	$CanvasLayer/Meter.value = dashmeter
	...
	if dashmeter < 15:
		$CanvasLayer/Meter.tint_progress = Color(0.350586, 0.350586, 0.350586)
	elif dashmeter < 60:
		$CanvasLayer/Meter.tint_progress = Color(0, 0.996078, 1)
	elif dashmeter == 60:
		$CanvasLayer/Meter.tint_progress = Color(1, 0.896484, 0)
		$CanvasLayer/Meter/FULLREADY.visible = true
		superdash = true
	...
	if is_on_floor() && !leaving:
		motion.y = 0
	
	
	#translation.x = clamp(translation.x, Globals.camera.limit_left + 20, Globals.camera.limit_right)
	...
	if $Rspr.animation == "Fjump" and !is_hit:
		...
		if $Rspr.flip_h == true:
			$Rspr.rotation_degrees.z += 8
		if $Rspr.flip_h == false:
			$Rspr.rotation_degrees.z -= 8
	else :
		$Rspr.rotation_degrees.z = 0
...
#					dashfx.translation = translation
...
#				else: comebacktimer -= 1
			...
			if Kright and !is_hit and Globals.moveenabled == 1 || (movright):
				...
				get_node("groundatk").set_scale(Vector3(1, 1, 1))
				get_node("groundatk2").set_scale(Vector3(1, 1, 1))
				get_node("attack3hit").set_scale(Vector3(1, 1, 1))
			...
			if (Kdash and  dashmeter >= 15 and !is_hit and Globals.moveenabled == 1) || (dash):
				...
				if dashes > 0:
					...
					if not is_on_floor():
						...
						dustinst.translation = translation
			...
			if is_on_floor():
				...
				if atkpts == 3 and (Kattack and not leaving and Globals.moveenabled == 1) or (attack and not leaving):
					...
					atkfxinst.translation.x = global_translation.x
					atkfxinst.translation.y = global_translation.y
		...
		#if translation.y > Globals.camera.limit_bottom + 24: //disconnect the player from the camera on death plane instead
			#Globals.hpcount = 0
...
func _on_gtimer_timeout():
	if dashing or zooming or jumpdash:
		...
		var this_ghost = preload("res://Stages/ghost3d.tscn").instance()
		...
		if wavedashing:
			this_ghost = preload("res://Stages/wbghost3d.tscn").instance()
			get_parent().add_child(this_ghost)
			this_ghost.mode = 1
		else :
			
			this_ghost.mode = 0
		this_ghost.translation = translation
...
func _on_Deathwait_timeout():
	...
	if Globals.lives > 0:
		translation.x = Globals.checkpX
		translation.y = Globals.checkpY
...
func _on_FULLREADY_animation_finished():
	$CanvasLayer/Meter/FULLREADY.visible = false
	pass # Replace with function body.
